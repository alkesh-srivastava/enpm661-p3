...
global_explore_z = list()  # This either
...
def move_possible(maze, pos, clearance):
    all_moves(pos)
    i, j, k = pos
    num_rows = len(maze)
    num_cols = len(maze[0])
    if (0 <= i < num_rows and 0 <= j < num_cols and not (obstacle(i, j, clearance))):
        all_moves(pos)

    return 0 <= i < num_cols and 0 <= j < num_rows and not (obstacle(i, j, clearance))
